...
//       const state = await services.createUser(services.getKCUser(u), req);

//       if (state) {
//         const kcToken = await services.autoLogin(u.email + "", req.body.password);
//         const PL = getPayload(jwt, kcToken.access_token, PUBLIC_KEY + "");
...
//     }
//   },
//   createUser: async (ud: any, req: Request) => {
...
//     return u;
//   },
//   registerWithKC: async (req: Request, res: Response) => {
//     const userRepository = req.DB.getRepository(User);
//     const email = req.body.email;
//     const user = await userRepository.findOne({
//       where: { email }
...
//     u.lastName = req.body.lastname;
//     u.password = req.body.password;
//     const { PUBLIC_KEY, KEYCLOAK_SECRET, KEYCLOAK_RESOURCE, KEYCLOAK_SERVER_URL, KEYCLOAK_REALM } = process.env;
//     try {
//       const token = await services.getAdminToken(KEYCLOAK_SERVER_URL + "", KEYCLOAK_REALM + "", KEYCLOAK_RESOURCE + "", KEYCLOAK_SECRET + "");
//       const state = await services.createUserForKC(KEYCLOAK_SERVER_URL + "", KEYCLOAK_REALM + "", token, services.getKCUser(u));
//       if (state) {
//         const kcToken = await services.autoLogin(u.email + "", req.body.password);
//         const PL = getPayload(jwt, kcToken.access_token, PUBLIC_KEY + "");
//         u.password = await services.hashPassword(req.body.password);
...
//   createUserForKC: async (keycloakUrl: string, realm: string, adminToken: string, userData: any) => {
...
//     }
//   },
//   autoLoginKC: async (username: string, password: string): Promise<KCToken> => {
