/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package com.hist.BysApp.dao;

import java.util.List;

import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.Query;
import org.springframework.data.repository.query.Param;
import org.springframework.data.rest.core.annotation.RepositoryRestResource;
import org.springframework.web.bind.annotation.CrossOrigin;

import com.hist.BysApp.entities.promo.Frag_cours;
import com.hist.BysApp.entities.promo.PromoFrag;

import Palmares.PFModel;
import dto.PFrag;

@CrossOrigin("*")
@RepositoryRestResource
public interface PFragDao extends JpaRepository<PromoFrag, Long> {

	@Query("SELECT pf FROM PromoFrag pf WHERE promotion.id=:idp")
    List<PromoFrag> getFragByPromo(@Param("idp") Long idp);
	
	@Query("SELECT c FROM PromoFrag AS p JOIN p.frag_cours AS c   WHERE c.id_cours=:id ")
    List<Frag_cours> getCoursByProf(@Param("id") Long id);
    
    @Query("SELECT COUNT(pf) FROM PromoFrag pf WHERE promotion.id=:idp AND pf.base=true")
    Long countBasePeriod(@Param("idp") Long idp);
    
    @Query("SELECT  pf FROM PromoFrag pf WHERE promotion.id=:idp AND pf.base=true ORDER BY pf.code ASC")
    List<PromoFrag> findBasePeriod(@Param("idp") Long idp);
    
    @Query("SELECT new Palmares.PFModel(pf.id, pf.share_code) FROM PromoFrag pf WHERE promotion.id=:idp AND pf.base=true ORDER BY pf.code ASC")
	List<PFModel> findBasePeriodPF(@Param("idp") Long idp);
    
	@Query("SELECT new dto.PFrag(pf.id, pf.code, promotion.id) FROM PromoFrag pf WHERE pf.promotion.id=:id ")
    PFrag getPFragById(@Param("id") Long id);
	
	@Query("SELECT new dto.PFrag(pf.id, pf.code) FROM PromoFrag pf WHERE promotion.id=:idp")
    List<PFrag> getFByPromo(@Param("idp") Long idp);
}
